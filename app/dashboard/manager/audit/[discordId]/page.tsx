...
async function getNismaraTransportUser(discordId: string) {
  try {
    const res = await fetch(
      `${process.env.NISMARA_TRANSPORT_URL}/api/users/${discordId}`,
      {
        headers: {
...
export default async function AuditUserPage({
  ...
  const [discordMember, allRoles, nismaraData, lamaranHistory] =
    await Promise.all([
  ...
  const memberRoles =
    discordMember?.roles?.map((roleId: string) => {
      const roleInfo = roleMap.get(roleId);
      return {
        id: roleId,
        name: roleInfo?.name || "Unknown Role",
        color: roleInfo?.color
          ? `#${roleInfo.color.toString(16).padStart(6, "0")}`
          : "#99aab5",
      };
    }) || [];
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Mengecek informasi profil, role discord, dan riwayat di Nismara
            Transport
          </p>
        </div>
      </div>
                ...
                <div className="flex flex-col items-center space-y-4">
                  <Avatar className="h-24 w-24 ring-2 ring-primary/20 ring-offset-2 ring-offset-background">
                    {discordMember.user.avatar ? (
                      <AvatarImage
                        src={`https://cdn.discordapp.com/avatars/${discordMember.user.id}/${discordMember.user.avatar}.png`}
                      />
                    ) : null}
            ...
            <CardContent>
              {nismaraData?.user ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/30 rounded-lg border">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="font-bold text-2xl text-primary">
                        {Math.floor(nismaraData.user.nc).toLocaleString(
                          "id-ID",
                        )}
                      </p>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="pt-1">
                        <Badge
                          variant={
                            nismaraData.user.isDriver ? "default" : "secondary"
                          }
                        >
                          {nismaraData.user.isDriver
                            ? "Aktif (Driver)"
                            : "Non-Aktif"}
                        </Badge>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="rounded-lg border p-4 flex flex-col gap-2 shadow-sm transition-colors hover:bg-muted/30">
                        ...
                        <span
                          ...
                          style={{
                            color:
                              nismaraData.user.truckyRankColor || "inherit",
                          }}
                        >
                      ...
                      <div className="rounded-lg border p-4 flex flex-col gap-2 shadow-sm transition-colors hover:bg-muted/30">
                        ...
                        <span
                          ...
                          style={{
                            color:
                              nismaraData.user.truckyRoleColor || "inherit",
                          }}
                        >
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                          <div className="space-y-1.5 p-3 rounded-md bg-muted/20 border">
                            ...
                            <p className="font-bold text-lg">
                              {nismaraData.userStats.jobs.total}
                            </p>
                          </div>
                          <div className="space-y-1.5 p-3 rounded-md bg-green-500/10 border border-green-500/20">
                            ...
                            <p className="font-bold text-lg text-green-700 dark:text-green-400">
                              {nismaraData.userStats.jobs.completed}
                            </p>
                          </div>
                          <div className="space-y-1.5 p-3 rounded-md bg-destructive/10 border border-destructive/20">
                            ...
                            <p className="font-bold text-lg text-destructive">
                              {nismaraData.userStats.jobs.canceled}
                            </p>
                          </div>
                          <div className="space-y-1.5 p-3 rounded-md bg-muted/20 border">
                            <p className="text-xs text-muted-foreground font-medium">
                            ...
                            <p className="font-bold text-lg">
                              {Math.floor(
                                nismaraData.userStats.jobs.distanceKm,
                ...
                <div className="text-center p-8 text-muted-foreground border border-dashed rounded-lg bg-muted/10 flex flex-col items-center justify-center min-h-[300px]">
                  ...
                  <p className="font-medium text-lg text-foreground/70">
                    Tidak ada data di Nismara Transport
                  </p>
                  <p className="text-sm mt-1 max-w-sm">
                    User ini belum terdaftar atau belum menghubungkan akun
                    Discord dengan web Nismara Transport.
                  </p>
                </div>
              )}
            </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  {nismaraData.leaveHistory.map((leave: any) => (
                    <div
                      key={leave._id}
                      className="border rounded-lg p-4 space-y-3 transition-colors hover:bg-muted/20"
                    >
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <p className="font-semibold">{leave.reason}</p>
                          <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                            <User className="h-3 w-3" /> Oleh:{" "}
                            {leave.managerName}
                          </p>
                        </div>
                        <Badge
                          variant={
                            leave.status === "deactivated"
                              ? "secondary"
                              : "default"
                          }
          ...
          {lamaranHistory && lamaranHistory.length > 0 && (
            <Card className="shadow-sm">
              <CardHeader className="pb-4">
                ...
                <CardDescription>
                  Daftar posisi yang pernah dilamar oleh user ini
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                    ...
                    >
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <p className="font-semibold text-primary">
                            {lamaran.lowonganTitle}
                          </p>
                          <p className="text-xs text-muted-foreground mt-1">
                            Dilamar pada:{" "}
                            {new Date(lamaran.appliedAt).toLocaleDateString(
                              "id-ID",
                              {
                                day: "numeric",
                                month: "long",
                      ...
                      <div className="pt-2 border-t flex items-center justify-between">
                        ...
                        <Button
                          ...
                          asChild
                        >
                          <Link href={`/dashboard/lamaran/${lamaran._id}`}>
                            Lihat Detail
                          </Link>
                        </Button>
                      </div>
