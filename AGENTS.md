...
13. **Wajib Menambahkan Metadata Pada Setiap Page Baru**: Setiap kali membuat page baru (baik publik maupun tidak/dashboard), **wajib** menyertakan metadata (seperti `title`) untuk memastikan tampilan tab browser yang baik bagi pengguna, serta SEO yang optimal (terutama untuk halaman publik). Ingat aturan Next.js: `metadata` hanya bisa diekspor dari Server Component. Jika page tersebut merupakan Client Component (`"use client"`), buatlah file `layout.tsx` di dalam folder yang sama khusus untuk mengekspor metadata tersebut.
